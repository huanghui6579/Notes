...
public class NoteLinkify {
    private static final String TAG = "NoteLinkify";
    /**
    ...
    public static final boolean addLinks(Spannable text, int mask) {
        return addLinks(text, mask, URLSpan.class);
    }
     ...
     *  attached to the Spannable, to avoid problems if you call it
     *  repeatedly on the same text.
     *  @param spanType URLSpan class
     */
    public static final boolean addLinks(Spannable text, int mask, Class<? extends URLSpan> spanType) {
        if (mask == 0) {
            return false;
        }

        URLSpan[] old = text.getSpans(0, text.length(), spanType);
     ...
     *  LinkMovementMethod.
     */
    public static final boolean addLinks(TextView text, int mask, Class<? extends URLSpan> spanType) {
        if (mask == 0) {
            return false;
        }
        ...
        if (t instanceof Spannable) {
            if (addLinks((Spannable) t, mask, spanType)) {
    ...
    private static final void applyLink(LinkSpec spec, Spannable text, Class<? extends URLSpan> spanType) {
        URLSpan span = null;
        if (spanType != null) {
            try {
                Constructor<? extends URLSpan> constructor = spanType.getConstructor(String.class);
                if (constructor != null) {
                    span = constructor.newInstance(spec.url);
                }
            } catch (Exception e) {
                Log.e(TAG, "----applyLink---error----" + e.getMessage());
            }
            if (span == null) {
                span = new URLSpan(spec.url);
            }
        } else {
            span = new URLSpan(spec.url);
        }
    ...
    private static final void applyLink(String url, int start, int end, Spannable text) {
        applyLink(url, start, end, text, null);
    }

    private static final void applyLink(String url, int start, int end, Spannable text, Class<? extends URLSpan> spanType) {
        LinkSpec spec = new LinkSpec();
        spec.url = url;
        spec.start = start;
        spec.end = end;
        applyLink(spec, text, spanType);
    }
